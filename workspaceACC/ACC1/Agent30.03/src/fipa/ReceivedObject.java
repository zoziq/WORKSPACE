 ...
 * 
 * A new received parameter is added to the envelope by each ACC that the message
 * passes through. Each ACC handling a message must add a completed received 
 * parameter. If an ACC receives a message it has already stamped, it is free to discard 
 * the message without any need to generate an error message.
