...
public class OdaOntoloji {
 ...
 static	int counter;
	...
	public OdaOntoloji() {
		...
		m.read(NS);
		m.prepare();
		m.setNsPrefix("Oda", "http://www.semanticweb.org/ontologies/2011/3/Oda.owl#");
	}
	...
	public ArrayList getPlan(String tip) {
		...
		String sorgu = "PREFIX rdf:<http://www.w3.org/1999/02/22-rdf-syntax-ns#> "
				...
				+ "SELECT ?odaNo ?ucret"
				+ " WHERE {"
				+ "?x Oda:tip '"+tip+"'.?x Oda:Durum 'Boþ'. ?x Oda:odaNo ?odaNo. ?x Oda:ucret ?ucret}";
		...
		while (results.hasNext()) {
			...
			System.out.println("result " + address.toString()+" "+address1.toString());
