...
public class OtelOntoloji {
 ...
 static	int counter;
	...
	public OtelOntoloji() {
		...
		m.read(NS);
		m.prepare();
		m.setNsPrefix("Otel", "http://www.semanticweb.org/ontologies/2011/3/Otel.owl#");
	}
	...
	public ArrayList getPlan(String deger) {
		...
		String sorgu = "PREFIX rdf:<http://www.w3.org/1999/02/22-rdf-syntax-ns#> "
				...
				+ "SELECT ?s  "
				+ " WHERE {"
				+ "?s Otel:yer '"+yer+"'.?s Otel:yildiz '"+yildiz+"'}";
...
//		Individual spraktv=m.createIndividual("Spor_Aktiviteleri"+counter,sporakt);
//		Individual su_spor=m.createIndividual("Su_Sporu"+counter,suspor);
//		Individual kara_spor1=m.createIndividual("Kara_Sporu"+counter,karaspor);
...
//	otel.addProperty(obj1,hvz);
	...
	public String seriOtel(Individual ind)
	{
		...
		sb.append(ind.getProperty(m.getDatatypeProperty(xmlbase+"yer")).getString()+" ");
		String u=ind.getProperty(m.getObjectProperty(xmlbase+"sahiptir")).getObject().asResource().getURI();
		Individual in=m.getIndividual(u);
		sb.append(in.getURI()+" ");
		sb.append("sahiptir ");
		sb.append(in.getProperty(m.getDatatypeProperty(xmlbase+"ad")).getString()+" ");
		sb.append(in.getProperty(m.getDatatypeProperty(xmlbase+"adres")).getString()+" ");
		System.out.println("ad "+in.getProperty(m.getDatatypeProperty(xmlbase+"ad")).getString());
		System.out.println("adres "+in.getProperty(m.getDatatypeProperty(xmlbase+"adres")).getString());
		System.out.println(" yer "+ind.getProperty(m.getDatatypeProperty(xmlbase+"yer")).getString());
