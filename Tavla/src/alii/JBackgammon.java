...
//            buttonState[i] = false;
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getActionCommand().equals(MakeMove)) {
            if (hamlesiVarmi()) {
                ...
                DoRoll(dice1, dice2);
                openCloseDice();  //static diceID ye göre kullanılan zarı kapatıyor.
                if (rolledDiceCounter == 100) {
                    yenizarlar(true);
                } else if (rolledDiceCounter == 90) {
                    ResetDiceSet();
                    yenizarlar(false);
                }
    ...
    private void ResetDiceSet() {
        ...
        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 10; j++) {
                ...
                dicea = rdice.nextInt(6) + 1;
                diceb = rdice.nextInt(6) + 1;
                yedekDiceButton[i][j].diceID = i + (j * 10);
    ...
    private void yenizarlar(boolean tumzarlar) {
        if (tumzarlar) {
            for (int i = 0; i < 10; i++) {
                for (int j = 0; j < 10; j++) {
                    DiceButton[i][j] = yedekDiceButton[i][j];
                    DiceButton[i][j].diceID = i + (j * 10);
            ...
            for (int i = 0; i < 10; i++) {
                System.out.println("yedek kont" + yedekDiceButton[i][0].diceID);
                DiceButton[i][0].diceID = yedekDiceButton[i][0].diceID;
