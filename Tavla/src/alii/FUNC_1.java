...
public class FUNC_1 implements YapayZeka {
	...
	private int zar1;
	private int zar2;
	private int kiranZar;
	private int kiranKonum;
    static int dice1;
    static int dice2;
    static int diceID;
...
//    	for (int i = 0; i < 25; i++) {
//    		System.out.print(rakip_pullar[i] + " ");
//    	}
//    	
    	int konum = 1;
    	int kontrol = 0;
    	...
    	if (benim_pullar[25] == 1) {//kirigi sokabiliyor muyum
    	...
    	if (benim_pullar[25] >= 2) {//kirigi sokabiliyor muyum
    		while(zarID != diceNodeList.size()-1) {
    			if (diceNodeList.get(zarID).getDice1() != diceNodeList.get(zarID).getDice2()) {
    	...
    	if (kontrol == 0) {
        	...
        	for (int i = 1; i < 12; i++) {
    			if (benim_pullar[i] > 0) {
    				for (int j = i+1; j < i+6; j++) {
    					if (rakip_pullar[j] == 1) {
    						if ((diceNodeList.get(zarID).getDice1() == j-i || diceNodeList.get(zarID).getDice2() == j-i)) {
    							kiranZar = zarID;
    							kiranKonum = i;
    							breakKontrol = 1;
    							break;
        	...
        	System.out.println(kiranKonum);
        	System.out.println(kiranZar);
            	...
            	if (kontrol != 3) {
            		...
            		while(konum<25) {//tekten oyna
            			if (benim_pullar[konum] == 1) {
            				if (buHamleOlurmu(diceNodeList.get(zarID).getDice1(), konum) && buHamleOlurmu(diceNodeList.get(zarID).getDice1(),konum+diceNodeList.get(zarID).getDice1())) {
            					zar1 = diceNodeList.get(zarID).getDice1();
            					zar2 = diceNodeList.get(zarID).getDice2();
            					calculate();
            					Play1(konum, diceNodeList.get(zarID).getDice1());
