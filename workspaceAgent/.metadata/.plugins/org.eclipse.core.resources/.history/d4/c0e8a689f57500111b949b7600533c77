...
public class Scheduler extends Thread {
	...
	public void run() {
		while (true) {
			...
			try {
				...
				// System.out.println("scheduler calisirken hata: " +
				// e.toString());
				e.printStackTrace();
			}
		}
	...
	private void linkleriKontrolEt(Behaviour gelenBeh, int msgNum) {
		...
		for (Link link : gelenBeh.getLinks()) {
			if (link.getClass().getName().contains("InheritanceLink")) {
				...
				System.out
						.println("Scheduler InheritanceLink Sender Provision Name: "
								+ il.getSenderProvision().getprovName()
										.toString());
	...
	private void uygunKuyruklaraGonder(Behaviour b, int msgNum) {
		...
		for (int i = 0; i < tasks.size(); i++) {
			...
			if (tasks.get(i).getClass().getSuperclass().getName().contains(
					"Action")) {

				System.out.println("Scheduler uygunKuyruk() Task Name: "
						+ tasks.get(i).getName());
				if (tasks.get(i).isAllProvisionsAreSet()) {
					System.out
							.println("Scheduler uygunKuyruk() Action ID: "
									+ tasks.get(i).getId()
									+ " tüm Provision'lar set edildi.");
					a = 1;
					System.out.println("Scheduler uygunKuyruk() list size "
							+ linkList.size());
