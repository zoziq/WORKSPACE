...
public class AgentManagementSystem implements AmsInterface {
	...
	public AgentManagementSystem() {
		...
		model.read(NS);
		model.prepare();
		model.setNsPrefix("Agent",
				"http://www.semanticweb.org/ontologies/2010/11/Agent.owl#");
	...
	public void deregister(String agentName) {
		...
		String sorgu = "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> "
				...
				+ "WHERE {"
				+ "?x Agent:name '"
				+ agentName + "'." + "?x Agent:address ?address" + "}";
	...
	public boolean registerAgent(FipaMessage fm) {
		...
		String name = fm.getAcl().getContent().getActions().get(0)
				.getArguments().get(0).toString();
		String ownership = fm.getAcl().getContent().getActions().get(0)
				.getArguments().get(2).toString();
		String state = "active";
		String id = null;
		String address = fm.getAcl().getContent().getActions().get(0)
				.getArguments().get(1).toString();
		String namespace1 = "AID";
		System.out.println("ams icinde "
				+ fm.getAcl().getContent().getActions().get(0).getArguments()
						.get(0).toString());
		...
		ad.addProperty(model.getDatatypeProperty(xmlbase + "ownership"),
				ownership);
		...
		ad1
				.addProperty(model.getDatatypeProperty(xmlbase + "address"),
						address);
	...
	public String sparql(FipaMessage fm) {
		...
		Query query = QueryFactory.create(fm.getAcl().getContent().getActions()
				.get(0).getArguments().get(0).toString());
		...
		while (results.hasNext()) {
			...
			RDFNode address = soln.get(fm.getAcl().getContent().getActions()
					.get(0).getAct());
